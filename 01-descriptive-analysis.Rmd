...
**This script generates descriptive statistics for variables collected at baseline (visit 1).**

The data columns were not analysed:  
- `pain` and `pain score`: related to SN itself, therefore not relevant at baseline when everyone was SN free.  

- `*_record`: inconsistent patient records.  

- `hba1c_percent` and `vitaminB12_pmol.l`: These data were only used to classify individuals as having diabetes or vitamin B12 deficiency when cleaning the data.  

- `ID`, `visit_number`, `visit_day`, `hivsn_present`, `visit_months`, `sn`: only provide sorting and grouping information. 
